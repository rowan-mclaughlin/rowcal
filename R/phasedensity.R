...
#' @param ... Additional arguments to be passed to the `MCdensity.rowyears` function.
#'
...
#'
#' @export
phasedensity <- function(siteids, datelist, h=30, ...) {
  phaselabels <- paste(siteids, phaser(siteids, datelist, h=h),sep='_')
  uniquephases <- which(!duplicated(phaselabels))
  out<-MCdensity(datelist[uniquephases], ...)
