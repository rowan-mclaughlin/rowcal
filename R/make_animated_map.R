...
#' @examples
#' \dontrun{
#' coast <- rgdal::readOGR("path/to/coastline.shp") # Load coastline data
#' data <- data.frame(
...
#' )
#' make_animated_map(data, coast, out = "Radiocarbon_Map.pdf", pt.cex = 1.2, threshold = 0.01)
#' }
#' 
...
make_animated_map<-function(DATA,coast,out='output.pdf',pt.cex=1,pt.col=rgb(1,0,0,0.4),threshold=0.002,dropempty=FALSE,BP=FALSE,width=6,height=6,...) {
  ...
  ts<-timematrix(DATA[,3:5])
  stmap<-cbind(DATA[,1:2],t(ts$timematrix))
  colnames(stmap)<-c('X','Y',ts$calyears)
  print('Drawing map')
